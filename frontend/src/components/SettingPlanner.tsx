...
export const SettingPlanner: React.FC<SettingPlannerProps> = ({
  ...
  const canManageSectors = currentUser?.role === 'Gym Manager';
  ...
  const [sectorType, setSectorType] = useState<
    ...
    >('bouldering');
  const [sectorMaxCapacity, setSectorMaxCapacity] = useState(20);
  const [sectorColorCode, setSectorColorCode] = useState('#ff4d00');
...
if (totalPlannedRoutes !== Number(targetRouteCount)) {
  ...
  onAddSector({
    name: sectorName,
    type: sectorType,
    maxCapacity: sectorMaxCapacity,
    colorCode: sectorColorCode,
  });

  setSectorName('');
  setSectorType('bouldering');
  setSectorMaxCapacity(20);
  setSectorColorCode('#ff4d00');
  };
  ...
  {canManageSectors && (
    <button
      ...
      className="bg-white hover:bg-zinc-50 text-zinc-700 border border-zinc-300 px-3.5 py-1.5 rounded-lg font-bold text-xs transition shadow-xs cursor-pointer shrink-0"
    >
      Zarządzaj sektorami
    </button>
  )}
...
<div className="pt-2.5 border-t border-zinc-100 flex items-center justify-between gap-2 text-xs">
  ...
  {session.status === 'completed' && (
    <span className="w-full text-center text-xs text-emerald-800 font-mono font-bold bg-emerald-50 border border-emerald-200 py-1.5 rounded-lg">
      ...
      {/* Modal: Sector Management */}
      {isSectorModalOpen && canManageSectors && (
          ...
          <div className="bg-white border border-zinc-200 rounded-xl max-w-md w-full p-5 shadow-2xl relative space-y-4 text-zinc-800">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-500 mt-1">
                Sektory dostępne w Twoim obiekcie.
              </p>
            </div>
              ...
              {sectors.length === 0 ? (
                <div className="border border-dashed border-zinc-300 rounded-lg p-4 text-center">
                  <p className="text-xs text-zinc-500">
                    Nie masz jeszcze żadnych sektorów.
                  </p>
                </div>
              ) : (
                sectors.map((sector) => (
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <div>
                        ...
                        <div className="text-[11px] text-zinc-500">
                          {sector.type} · pojemność: {sector.maxCapacity}
                        </div>
                      </div>
                    </div>
                  </div>
...
>
  ...
  <div>
    <label className="block text-zinc-700 font-semibold mb-1 text-xs">
      Nazwa sektora *
    </label>
    ...
    <input
      ...
      className="w-full bg-zinc-50 border border-zinc-200 rounded-lg px-2.5 py-1.5 text-zinc-900 text-xs focus:outline-none focus:border-[#ff4d00]"
    />
  </div>
  ...
  <div className="grid grid-cols-2 gap-2.5">
    <div>
      ...
      <select
        ...
        onChange={(e) =>
          setSectorType(
            e.target.value as 'bouldering' | 'rope_wall' | 'training'
          )
        }
        className="w-full bg-zinc-50 border border-zinc-200 rounded-lg px-2.5 py-1.5 text-zinc-900 text-xs focus:outline-none focus:border-[#ff4d00]"
      >
    ...
    <div>
      ...
      <input
        ...
        min="1"
        max="100"
        value={sectorMaxCapacity}
        onChange={(e) => setSectorMaxCapacity(Number(e.target.value))}
        className="w-full bg-zinc-50 border border-zinc-200 rounded-lg px-2.5 py-1.5 text-zinc-900 text-xs font-mono font-bold focus:outline-none focus:border-[#ff4d00]"
      />
    </div>
  </div>
  ...
  <div>
    ...
    <div className="flex items-center space-x-2">
      <input
        type="color"
        value={sectorColorCode}
        onChange={(e) => setSectorColorCode(e.target.value)}
      ...
      <input
        ...
        value={sectorColorCode}
        onChange={(e) => setSectorColorCode(e.target.value)}
        className="flex-1 bg-zinc-50 border border-zinc-200 rounded-lg px-2.5 py-1.5 text-zinc-900 text-xs font-mono focus:outline-none focus:border-[#ff4d00]"
      />
    </div>
  </div>
  ...
  <div className="flex justify-end">
    <button
      type="submit"
      className="px-4 py-1.5 rounded-lg bg-[#ff4d00] hover:bg-[#e04400] text-white font-bold transition shadow-xs text-xs"
    >
      Dodaj sektor
    </button>
  </div>
            ...
            <div className="flex justify-end pt-2">
              <button
                type="button"
                onClick={() => setIsSectorModalOpen(false)}
                className="px-3 py-1.5 rounded-lg bg-zinc-100 text-zinc-700 font-semibold hover:bg-zinc-200 transition text-xs"
              >
                Zamknij
              </button>
            </div>
