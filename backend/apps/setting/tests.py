...
class SettingSessionViewSetTest(APITestCase):
    ...
    def test_user_cannot_create_session_with_sector_from_another_gym(self):
        self.client.force_authenticate(user=self.user_a)
        ...
        'sector': str(self.sector_b.id),
        'scheduled_date': '2026-08-24',
        'lead_setter': str(self.setter_b.id),
    }
...
def test_user_cannot_create_task_with_setter_from_another_gym(self):
        self.client.force_authenticate(user=self.user_a)
        ...
        self.assertEqual(response.status_code, 400)
        self.assertIn('setter', response.data)


def test_user_cannot_create_reset_log_for_another_gym(self):
        self.client.force_authenticate(user=self.user_a)
